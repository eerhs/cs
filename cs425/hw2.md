...
from instructor
join teaches
using(course_id, sec_ id, semester, year)
...
``` sql
select course_id, sec_id, semester, year, avg(tot_cred) as avg_total_credits
...
and course.dept_name like 'Math'
  ) < 5;
```
...
and instructor.id = teaches.id and grade is not null
